...
use openindex_sdk::{issuance::{error::IssuanceError, pda::find_issuance_signer_address, state::IssuanceConfig}, openindex::{error::ProtocolError, instruction::ProtocolInstruction, pda::find_module_address, state::IndexMints}, require};
use solana_program::{account_info::{next_account_info, AccountInfo}, entrypoint::ProgramResult, instruction::{AccountMeta, Instruction}, program::invoke_signed, program_error::ProgramError, pubkey::Pubkey};

pub fn issue(program_id: &Pubkey, accounts: &[AccountInfo], index_id: u64, amount: u64) -> ProgramResult {
    ...
    for hook in issuance_config.allowed_hooks.iter() {
        ...
        require!(hook_account.key == hook, IssuanceError::IncorrectHookAccount.into());
    ...
    let (module_pda, module_bump) = find_module_address(openindex_program_account.key, &issuance_signer_pda);
    ...
    for (idx, mint) in index_mints_data.mints.iter().enumerate() {
        ...
        cpi_accounts.push(  AccountMeta::new_readonly(*component_mint_account.key, false));
