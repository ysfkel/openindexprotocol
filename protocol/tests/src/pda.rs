use open_index_lib::seeds::{
    CONTROLLER_GLOBAL_CONFIG_SEED, CONTROLLER_SEED, INDEX_SEED, PROTOCOL_SEED,
};
...
pub fn get_index_pda(program_id: &Pubkey, controller_id: u64, index_id: u64) -> (Pubkey, u8) {
    let (pda, bump) = Pubkey::find_program_address(
        &[
            INDEX_SEED,
            &controller_id.to_le_bytes().as_ref(),
